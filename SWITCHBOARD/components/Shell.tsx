...
import {Activity,LogOut,Network,ScrollText,Settings,ShieldCheck,Users,Workflow,Zap,KeyRound} from 'lucide-react';
...
export function Shell({children,title,eyebrow,actions}:{children:React.ReactNode;title:string;eyebrow?:string;actions?:React.ReactNode}){
  ...
  return <div className="app-shell"><header className="workspace-header"><Link href="/workflows" className="cp-brand"><Network size={16}/><span>SWITCHBOARD</span><i>AUTOMATION</i></Link><nav className="cp-nav">{sections.slice(0,4).map(({href,label})=><Link key={href} className={path===href||path.startsWith(href+'/')?'active':''} href={href}>{label}</Link>)}</nav><div className="header-actions">{actions}<span className="api-pill"><Activity size={12}/> LIVE</span><button className="ghost-icon" onClick={()=>setOpen(v=>!v)}>{String(session.user?.name||'S').slice(0,1).toUpperCase()}</button></div>{open&&<div className="cp-menu"><b>{session.user?.name}</b><small>{session.user?.role}</small>{sections.slice(4).map(({href,label,icon:Icon})=><Link key={href} href={href}><Icon size={13}/>{label}</Link>)}<button onClick={logout}><LogOut size={13}/> LOG OUT</button></div>}</header><main className="workspace"><div className="page-titlebar"><div><p className="eyebrow">{eyebrow||'SWITCHBOARD'}</p><h1>{title}</h1></div></div><section className="workspace-content">{children}</section></main></div>
}
