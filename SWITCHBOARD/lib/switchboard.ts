...
export async function sb<T=any>(path:string,init?:Options):Promise<T>{const session=getSession();const headers:any={'content-type':'application/json',...(init?.headers||{})};const r=await fetch(`/api/switchboard${path}`,{...init,headers,credentials:'same-origin'});const data=await r.json().catch(()=>({}));if(r.status===401&&init?.auth!==false&&typeof window!=='undefined'){clearSession();if(!location.pathname.startsWith('/login'))location.href='/login'}if(!r.ok)throw Object.assign(new Error(apiMessage(data,r.status)),{issues:data.issues||data.details?.issues||[]});return data}
