...
Next.js remains on version 15. PostCSS is overridden to the patched 8.5 line for [its source-map loading advisories](https://github.com/advisories/GHSA-fxqj-rqcc-2cmp). Prisma remains on version 6 with deepmerge-ts 8 for [the recursive merge fix](https://github.com/advisories/GHSA-ggr8-5vv4-36mx). Nodemailer is updated to 10 and email nodes disable file/URL content loading. CI checks dependency advisories and exercises SMTP over a disposable local test server. External provider delivery still needs the actual provider configuration.
