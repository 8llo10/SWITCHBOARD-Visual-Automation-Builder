...
import {CheckCircle2,Mail,Network,RefreshCw,XCircle} from 'lucide-react';
...
function VerifyEmailContent(){
  ...
  const[state,setState]=useState<'idle'|'working'|'verified'|'error'>(token?'working':'idle');
  const[message,setMessage]=useState(token?'Verifying secure link…':'Check your inbox for the verification link.');
  const[busy,setBusy]=useState(false);

  useEffect(()=>{
    if(!token)return;
    sb<any>('/auth/verify-email',{method:'POST',body:JSON.stringify({token}),auth:false} as any)
      .then(()=>{
        setState('verified');
        setMessage('Email verified. Access is now enabled.');
  ...
  async function resend(){
    ...
    setBusy(true);
    try{
      const r=await sb<any>('/auth/resend-verification',{method:'POST',body:JSON.stringify({email}),auth:false} as any);
      setMessage(r.message||'Verification request accepted.');
    }catch(e:any){
...
export default function VerifyEmail(){
  return <Suspense fallback={<VerifyLoading/>}><VerifyEmailContent/></Suspense>;
}
