...
async function proxy(req:NextRequest,{params}:{params:Promise<{path:string[]}>}){
 ...
 try{
  ...
  const out=new NextResponse(upstream.status===204?null:text,{status:upstream.status,headers:{'content-type':upstream.headers.get('content-type')||'application/json'}});const rid=upstream.headers.get('x-request-id');if(rid)out.headers.set('x-request-id',rid);if(upstream.status===401||(joined==='auth/logout'&&req.method==='POST'))out.cookies.set(COOKIE,'',{httpOnly:true,secure:process.env.NODE_ENV==='production',sameSite:'lax',path:'/',maxAge:0});return out
 }catch{const out=NextResponse.json({error:'Backend unavailable'},{status:503});if(joined==='auth/logout'&&req.method==='POST')out.cookies.set(COOKIE,'',{httpOnly:true,secure:process.env.NODE_ENV==='production',sameSite:'lax',path:'/',maxAge:0});return out}
}
