...
async function executeConfigured(node:WorkflowNode,ctx:EngineContext,c:any):Promise<ExecutorResult>{
 ...
 if(kind==='health'){const url=c.url;if(!url)throw new Error('health requires url');const start=Date.now();try{const res=await fetch(url,{method:'GET',signal:AbortSignal.timeout(Number(c.timeoutMs||8000))});await res.body?.cancel();return {output:{url,status:res.status,ok:c.expectedStatus?res.status===Number(c.expectedStatus):res.ok,latencyMs:Date.now()-start}}}catch{return {output:{url,status:0,ok:false,error:'Health endpoint did not respond',latencyMs:Date.now()-start}}}}
 if(kind==='http'){
