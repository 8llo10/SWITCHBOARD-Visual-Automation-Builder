import {assertAllowedTarget} from '../../utils/targets.js';
...
async function cfg(node:WorkflowNode,ctx:EngineContext){const base=render(node.data.config||{},ctx) as any;if(base.credentialRef){const cred=await prisma.credential.findUniqueOrThrow({where:{id:String(base.credentialRef)}});const secret=decryptJson<any>(cred.encryptedData);delete base.credentialRef;for(const key of ['host','port','username','database','connectionString'])if(base[key]===''||base[key]===undefined)delete base[key];return {...secret,...base}}return base}
...
async function executeConfigured(node:WorkflowNode,ctx:EngineContext,c:any):Promise<ExecutorResult>{
 ...
 if(kind==='createUser'){
   ...
   const user=await prisma.directoryUser.upsert({where:{email},update:{fullName,department:c.department||getPath(ctx,'trigger.department')||null,title:c.title||getPath(ctx,'trigger.title')||null,active:true,custom:c.custom||getPath(ctx,'trigger.custom')||{}},create:{email,fullName,department:c.department||getPath(ctx,'trigger.department')||null,title:c.title||getPath(ctx,'trigger.title')||null,custom:c.custom||getPath(ctx,'trigger.custom')||{}}}); return {output:user};
 }
 ...
 if(kind==='health'){const url=c.url;if(!url)throw new Error('health requires url');assertAllowedTarget(new URL(url).hostname);const start=Date.now();try{const res=await fetch(url,{method:'GET',redirect:'manual',signal:AbortSignal.timeout(Number(c.timeoutMs||8000))});await res.body?.cancel();return {output:{url,status:res.status,ok:c.expectedStatus?res.status===Number(c.expectedStatus):res.ok,latencyMs:Date.now()-start}}}catch{return {output:{url,status:0,ok:false,error:'Health endpoint did not respond',latencyMs:Date.now()-start}}}}
 if(kind==='http'){
   if(!c.url)throw new Error('http requires url');const url=new URL(c.url);assertAllowedTarget(url.hostname);for(const [key,value] of Object.entries(c.queryParams||{}))url.searchParams.set(key,String(value));const requestBody=c.bodyType==='text'?String(c.body||''):c.bodyType==='form'?new URLSearchParams(c.body||{}).toString():JSON.stringify(c.body||{});const res=await fetch(url,{method:c.method||'GET',redirect:'manual',headers:{'Content-Type':c.bodyType==='text'?'text/plain':c.bodyType==='form'?'application/x-www-form-urlencoded':'application/json',...(c.headers||{}),...(c.secret?{[c.headerName||'Authorization']:`${c.prefix===undefined?'Bearer':c.prefix} ${c.secret}`.trim()}:{}) ,...(c.username&&c.password?{Authorization:`Basic ${Buffer.from(`${c.username}:${c.password}`).toString('base64')}`}:{})},body:['GET','HEAD'].includes((c.method||'GET').toUpperCase())?undefined:requestBody,signal:AbortSignal.timeout(Number(c.timeoutMs||15000))});const text=await res.text();let body:any=text;try{body=JSON.parse(text)}catch{} if(c.expectedStatus?res.status!==Number(c.expectedStatus):!res.ok&&!c.allowNon2xx)throw new Error(`HTTP request failed with status ${res.status}`);return {output:{status:res.status,headers:Object.fromEntries(res.headers.entries()),body}};
 }
 if(kind==='postgres'){
   if(!c.connectionString||!c.query)throw new Error('postgres requires connectionString and query');assertAllowedTarget(c.host||new URL(c.connectionString).hostname);const pool=new Pool({connectionString:c.connectionString,...(c.host?{host:String(c.host)}:{}),...(c.database?{database:String(c.database)}:{}),ssl:c.ssl?{rejectUnauthorized:true}:undefined,max:1,connectionTimeoutMillis:Number(c.timeoutMs||15000),statement_timeout:Number(c.timeoutMs||15000),query_timeout:Number(c.timeoutMs||15000)});try{const r=await pool.query(c.query,Array.isArray(c.params)?c.params:[]);return {output:{rowCount:r.rowCount,rows:r.rows}}}finally{await pool.end()}
 }
 if(kind==='email'){
   if(!(c.host||env.SMTP_HOST)||!(c.username||env.SMTP_USER)||!(c.password||env.SMTP_PASS))throw new Error('SMTP is not configured');assertAllowedTarget(c.host||env.SMTP_HOST!);const transporter=nodemailer.createTransport({host:c.host||env.SMTP_HOST,port:Number(c.port||env.SMTP_PORT),secure:Number(c.port||env.SMTP_PORT)===465,connectionTimeout:15000,socketTimeout:30000,auth:{user:c.username||env.SMTP_USER,pass:c.password||env.SMTP_PASS}});const info=await transporter.sendMail({disableFileAccess:true,disableUrlAccess:true,from:env.SMTP_FROM,to:String(c.to||getPath(ctx,'trigger.email')||''),subject:String(c.subject||'Welcome'),text:c.text?String(c.text):undefined,html:c.html?String(c.html):undefined,cc:c.cc?String(c.cc):undefined,bcc:c.bcc?String(c.bcc):undefined});return {output:{messageId:info.messageId,accepted:info.accepted}};
 }
 if(kind==='ssh'||kind==='powershell'){
   if(!c.host||!c.username||!c.command)throw new Error(`${kind} requires host, username and command`);assertAllowedTarget(String(c.host));return {output:await new Promise((resolve,reject)=>{const client=new SSHClient();let stdout='',stderr='';const timeout=setTimeout(()=>{client.end();reject(new Error('Remote command timed out'))},Math.min(120000,Number(c.timeoutMs)||30000));client.once('close',()=>clearTimeout(timeout));client.on('ready',()=>client.exec(kind==='powershell'?`powershell.exe -NoProfile -NonInteractive -EncodedCommand ${Buffer.from(String(c.command),'utf16le').toString('base64')}`:c.command,(err,stream)=>{if(err){client.end();return reject(err)}stream.on('close',(code:number)=>{client.end();code===0?resolve({code,stdout,stderr}):reject(new Error(`${kind} exited with code ${code}`))}).on('data',(d:Buffer)=>stdout+=d.toString());stream.stderr.on('data',(d:Buffer)=>stderr+=d.toString())})).on('error',reject).connect({host:c.host,port:Number(c.port||22),username:c.username,password:c.password,privateKey:c.privateKey,readyTimeout:10000})})};
 }
