...
test('template cannot traverse prototypes',()=>{assert.equal(getPath({},'constructor.name'),undefined);assert.equal(getPath({},'__proto__.polluted'),undefined)});
test('whole references preserve booleans, numbers and objects',()=>{assert.equal(render('{{trigger.ok}}',{trigger:{ok:false}}),false);assert.deepEqual(render('{{trigger.data}}',{trigger:{data:[1,2]}}),[1,2])});
