...
export async function run(workflowId:string,triggerType:string,payload:any){const workflow=await prisma.workflow.findUniqueOrThrow({where:{id:workflowId},select:{version:true,definition:true}});const r=await prisma.workflowRun.create({data:{workflowId,triggerType,triggerPayload:payload||{},context:{workflowVersion:workflow.version,definitionSnapshot:workflow.definition,runtime:null} as any}});void executeRun(r.id);return r}
